...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($resultado) {
        ...
        if ($resultado['tentativas_falhas'] >= 5) {
        ...
        if (password_verify($senha, $resultado['senha'])) {
            ...
            // Zera os erros
            $update_erro = $pdo->prepare("UPDATE login SET tentativas_falhas = 0 WHERE email = :email");
            ...
            $update_erro = $pdo->prepare("UPDATE login SET tentativas_falhas = CASE WHEN tentativas_falhas < 5 THEN tentativas_falhas + 1 ELSE tentativas_falhas END WHERE email = :email");
            $update_erro->bindParam(":email", $email);
            $update_erro->execute();

            // Consulta tentativas
            $consulta_erro = $pdo->prepare("SELECT tentativas_falhas FROM login WHERE email = :email");
            $consulta_erro->bindParam(":email", $email);
            $consulta_erro->execute();
            $consulta = $consulta_erro->fetch(PDO::FETCH_ASSOC);
            $erros = (int)$consulta["tentativas_falhas"];
