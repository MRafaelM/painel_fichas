...
if (isset($_POST['btn-trocar'])) {
    ...
    if ($rowsEmail) {
        ...
        $trocar_senha = $pdo->prepare("UPDATE login SET recuperar = :chave WHERE email = :email");
        ...
        if ($trocar_senha->execute()) {
            ...
            $mail->Body = "
                ...
                <p>Olá, <b>$nome</b>!</p>               
                <p><a href='https://pmsoledaders.inf.br/painel/login/atualizar.php?chave=$chave'>Clique aqui</a> para alterar sua senha.</p>
                <br>
            ...
            if (!$mail->send()) {
                ...
                $inserir_logs_user->bindParam(':user', $resultado['nome']);
                $inserir_logs_user->bindParam(':cpf', $resultado['cpf']);
                $inserir_logs_user->bindParam(':nivel', $resultado['nivel']);
