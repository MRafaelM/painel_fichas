...
$data_inicio = date('Y-m-d', strtotime('previous month', strtotime(date('Y-m-20')))); // Dia 20 do mês anterior
$data_fim = date('Y-m-d', strtotime('this month', strtotime(date('Y-m-20')))); // Dia 20 do mês atual

$consulta = $pdo->prepare("SELECT * FROM chamados WHERE DATE(data_abertura) BETWEEN ? AND ?");
$consulta->execute([$data_inicio, $data_fim]);
$chamados = $consulta->fetchAll(PDO::FETCH_ASSOC);

$consulta = $pdo->prepare("SELECT * FROM atendimentos_externos WHERE DATE(data_atendimento) BETWEEN ? AND ?");
$consulta->execute([$data_inicio, $data_fim]);
$telefones = $consulta->fetchAll(PDO::FETCH_ASSOC);

$consulta = $pdo->prepare("SELECT * FROM tabela_social WHERE DATE(data_atendimento) BETWEEN ? AND ?");
$consulta->execute([$data_inicio, $data_fim]);
...
<body>
    ...
    <div class="container">
        ...
        <h4>Relatório Mensal - <?php echo date('m/Y', strtotime($data_inicio)); ?> - <?php echo date('m/Y', strtotime($data_fim)); ?></h4>
