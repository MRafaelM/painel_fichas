...
$dia_atual = date('d'); // Dia atual

// Calcular a semana do mês correspondente ao dia atual
$semana_do_mes = ceil($dia_atual / 7);

// Consulta para buscar os dados apenas da semana correspondente
$consulta = $pdo->prepare("SELECT * FROM chamados WHERE WEEKOFYEAR(data_abertura) = WEEKOFYEAR(:data) AND YEAR(data_abertura) = YEAR(:data)");
$consulta->bindParam(':data', $data_hoje);
$consulta->execute();
$chamados = $consulta->fetchAll(PDO::FETCH_ASSOC);

$consulta = $pdo->prepare("SELECT * FROM atendimentos_externos WHERE WEEKOFYEAR(data_atendimento) = WEEKOFYEAR(:data) AND YEAR(data_atendimento) = YEAR(:data)");
$consulta->bindParam(':data', $data_hoje);
$consulta->execute();
$telefones = $consulta->fetchAll(PDO::FETCH_ASSOC);

$consulta = $pdo->prepare("SELECT * FROM tabela_social WHERE WEEKOFYEAR(data_atendimento) = WEEKOFYEAR(:data) AND YEAR(data_atendimento) = YEAR(:data)");
$consulta->bindParam(':data', $data_hoje);
$consulta->execute();
...
foreach ($chamados as $chamado) {
    ...
    if (!isset($funcionarios[$funcionario])) {
        $funcionarios[$funcionario] = ['chamados' => 0, 'telefones' => 0, 'sociais' => 0];
    }
...
foreach ($social as $atendimento_social) {
    // Conta quantos atendimentos sociais cada funcionário realizou
    $funcionario = $atendimento_social['nome_funcionario'];
    if (!isset($funcionarios[$funcionario])) {
        $funcionarios[$funcionario] = ['chamados' => 0, 'telefones' => 0, 'sociais' => 0];
    }
...
<body>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12 text-center" style="margin-top: 20px;">
                <table class="mx-auto no-border">
                    <tr>
                        <td>
                            <img src="../assets/img/brasao.png" alt="Brasão" width="82" height="90">
                        </td>
                        <td>
                            <b>
        ...
        <div style="text-align: end;">
            <table class="no-border">
                <tr>
                    <td><i>&nbsp;&nbsp;Código controle:&nbsp; <?php echo $id_usuario; ?> </i></td>
                </tr>
            </table>
        </div>

        <h4>Relatório Semanal - Semana <?php echo $semana_do_mes; ?></h4>
    ...
    <div class="footer">
        ...
        <div class="footer-name"><?php echo $nome_usuario; ?></div>
    </div>
</body>
